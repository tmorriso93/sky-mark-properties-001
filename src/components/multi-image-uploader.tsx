...
export default function MultiImageUploader({
    ...
    //add images when uploading from dashboard, images are now dragable
    const handleDragEnd = (result: DropResult) => {
        ...
        const [reordedImage] = items.splice(result.source.index, 1);
        items.splice(result.destination.index, 0, reordedImage);
        onImagesChange(items);

    };

    //delete images when uploading from dashboard
                                         ...
                                         >
                                            <div className="bg-gray-100 rounded-lg flex gap-2  items-center overflow-hidden">
                                                ...
                                                <div className="flex items-center p-2">
                                                    <button className="text-red-500" onClick={() => handleDelete(image.id)}>
                                                        <XIcon />
                                                    </button>
