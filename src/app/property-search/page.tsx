...
export default async function PropertySearch({
    ...
    const maxPrice = isNaN(parsedMaxPrice) ? null : parsedMaxPrice; // ✅ Corrected here
  ...
  if (token) {
    ...
    return ( 
        <div className="max-w-screen-lg mx-auto">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mt-5 gap-5  max-md:px-5">
                {data.map((property) => {
                    const addressLines = [
                        property.address1,
                        property.address2,
                    ...
                    return (
                    <Card key={property.id} className=" overflow-hidden py-0 shadodw-md">
                    {/* <Card key={property.id} className="overflow-hidden"> */}
                        <CardContent className="px-0 pb-0">
                            <div className="h-40 relative bg-sky-50 text-zinc-400 flex flex-col">
                               ...
                               {!!property.images?.[0] && (
                                ...
                                {!property.images?.[0] && (
                                 <div className="flex flex-col items-center justify-center h-full text-center">
                                    <HomeIcon className=""/>
                                    <small className="">No Image</small>
                                 </div>
                                )}
                            </div>
